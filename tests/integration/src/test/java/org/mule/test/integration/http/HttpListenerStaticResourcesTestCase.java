...
public class HttpListenerStaticResourcesTestCase extends FunctionalTestCase
{
    ...
    {
        muleContext.getNotificationManager().addListener(new MessageProcessingStackReadyNotificationListener<MessageProcessingStackNotification>()
        {
            @Override
            public void onNotification(MessageProcessingStackNotification notification)
            {
                MessageProcessStack messageProcessStack = (MessageProcessStack) notification.getSource();
                messageProcessStack.getLastEventDebugLine();
            }
        });
